...
//   </Tooltip>
// );
const AboutTarget = ({ data, targetDetailsError,targetDetailsLoading, description }) => {
    ...
    <section id="introduction">
        ...
        {targetDetailsError &&
		 ...
		 (
            <>
			{
				data &&(
					<div>
            <span>Uniprot ID: <span className="text-sky-800" >{data.target_details.uniprot_id}</span> | </span> 
            <span>ENSGID: <span className="text-sky-800"> {data.target_details.ensembl_id} </span></span> </div>)
			}
            <div className="flex gap-32 mt-3">
              {/* Function Descriptions Section */}
              <div className="flex-1 ">
                <h2 className="text-lg font-medium subHeading">Function Descriptions</h2>
              ...
              {data &&<ProteinImage uniprot={data.target_details.uniprot_id}/>}
              </div>
            </div>
          </>
        )}
      ...
      <article id="taxonomy" className="mt-12 px-[5vw] bg-gray-50 py-20">
        ...
        <p className="italic font-medium mt-2">
          This section provides the NCBI unique identifier for the source
          organism and its taxonomy. This ensures understanding functional
          differences, especially when comparing targets across species.
        </p>
