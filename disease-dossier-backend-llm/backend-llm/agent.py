...
FUNCTIONS_WITH_MULTI_DF = """
Below are the dataframe descriptions and the top {number_of_head_rows} rows from each:
...
def _get_multi_prompt(
    dfs: Dict[str, pd.DataFrame],  # Change type to dict
    *,
    ...
    # Create dfs_head with keys and DataFrame heads
    if "dfs_head" in partial_prompt.input_variables:
        dfs_head = "\n\n".join(
            [f"### {key}:\n{df.head(number_of_head_rows).to_markdown()}" for key, df in dfs.items()]
        )
...
def _get_functions_multi_prompt(
    ...
    **kwargs
    ...
    dfs_descriptions_text = ""
    dfs_head_text = ""
    
    for df_key, df_val in dfs.items():
        description = df_descriptions.get(df_key, "No description provided.")
        # Convert top rows to markdown
        head_markdown = df_val.head(number_of_head_rows).to_markdown()

        # Add to a combined string
        dfs_descriptions_text += f"### {df_key}\nDescription: {description}\n\n"
        dfs_head_text += f"### {df_key}\n{head_markdown}\n\n"
    
    suffix = FUNCTIONS_WITH_MULTI_DF.format(
        number_of_head_rows=number_of_head_rows,
        dfs_descriptions=dfs_descriptions_text.strip(),
        dfs_head=dfs_head_text.strip(),
    )

    prefix = (prefix or MULTI_DF_PREFIX_FUNCTIONS).format(num_dfs=str(len(dfs)))
    # 3. Combine everything into one big system prompt
    system_prompt = prefix + suffix
...
def _get_functions_prompt(df: Any, **kwargs: Any) -> ChatPromptTemplate:
    return (
        _get_functions_multi_prompt(dfs=df, **kwargs)
        # if isinstance(df, list)
...
def create_pandas_dataframe_agent(
    ...
    **kwargs: Any,
    ...
    if not isinstance(df, dict):
        raise ValueError(f"Expected a dictionary, got {type(df)}")

    for key, dataframe in df.items():
        if not isinstance(dataframe, pd.DataFrame):
            raise ValueError(f"Expected pandas DataFrame for key '{key}', got {type(dataframe)}")
